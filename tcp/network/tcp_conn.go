...
func (receiver *TCPClient) Read() (int32, []byte, error) {
	...
	// 通过Read接口可以将buff中的内容填充到data参数表示的数据结构中
	...
	// Buffered 返回缓存中未读取的数据长度
	if int32(receiver.r.Buffered()) < length+receiver.head+receiver.tag {
		return 0, nil, err
	}
	// 读取消息真正的内容
	pack := make([]byte, int(receiver.head+length+receiver.tag))
	// Read从 b 中读出数据到 p 中，返回读出的字节数和遇到的错误。
